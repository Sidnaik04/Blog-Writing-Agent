...
export function Button({
  ...
  const base =
    "inline-flex items-center justify-center gap-2 font-medium transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:opacity-40 disabled:pointer-events-none rounded-lg";

  const variants = {
    primary: "bg-accent text-white hover:bg-accent-light active:scale-[0.98]",
    secondary:
      "bg-surface-2 text-ink hover:bg-surface-3 border border-rule active:scale-[0.98]",
  ...
  return (
    <Component
      ...
      disabled={!["button", "input"].includes(Component) ? undefined : disabled}
      {...props}
    >
      {children}
    </Component>
...
export function IconFilePdf({ size = 16, className = "" }) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      ...
      <path d="M10 12H8v5h1.5a1.5 1.5 0 0 0 0-3H8" />
      <path d="M16 12h-2v5h2a2 2 0 0 0 0-4z" />
    </svg>
  );
}

export function IconFileText({ size = 16, className = "" }) {
  return (
    <svg
      width={size}
      height={size}
